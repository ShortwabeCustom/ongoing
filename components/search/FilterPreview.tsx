...
export function FilterPreview({
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center px-4 py-2 bg-slate-50 border-b border-slate-200">
      {/* Status filters */}
      {filters.status?.map((status) => (
        <div
          key={`status-${status}`}
          className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-white border border-slate-200 rounded text-xs font-medium text-slate-700 hover:bg-slate-100"
        >
      ...
      {dateRangeLabel && (
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-white border border-slate-200 rounded text-xs font-medium text-slate-700 hover:bg-slate-100">
          <span>Fecha: {dateRangeLabel}</span>
          {onRemoveDateRange && (
            <button
      ...
      {onClearAll && (
        <button
          onClick={onClearAll}
          className="px-2.5 py-1 text-xs font-medium text-slate-600 hover:text-slate-900 focus-visible:ring-2 focus-visible:ring-indigo-500 rounded min-h-[24px]"
        >
          Limpiar todo
        </button>
      )}
