...
import { isLegacyStorageKey } from '@/lib/services/storage-service'
...
export async function GET() {
  try {
    ...
    const findingsList = findings.map((finding, index) => {
      ...
      // Process evidence: use legacy URLs directly, skip non-legacy for now (none in DB today)
      const evidenceList = finding.evidence
        .filter((ev) => isLegacyStorageKey(ev.storageKey))
        .map((ev) => ({
          url: ev.url || '',
